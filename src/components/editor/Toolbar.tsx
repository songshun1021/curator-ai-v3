  ...
  return (
    <div className="border-b border-white/70 bg-white/80 px-3 py-2 text-xs backdrop-blur-xl">
      <div className="flex flex-wrap items-center gap-2">
        {isGenerating ? (
          <button type="button" className="curator-button-danger" onClick={() => cancelGeneration()}>
            取消生成
          </button>
        ) : null}
        ...
        {currentFilePath === "/简历/主简历.json" ? (
          <>
            <button type="button" disabled={isGenerating} onClick={polishResume} className="curator-button-secondary gap-1">
              <Sparkles size={14} /> AI 润色
            </button>
            <button
              type="button"
              className="curator-button-secondary gap-1"
              onClick={async () => {
        ...
        {jobFolderPath ? (
          <>
            <button type="button" disabled={isGenerating} className="curator-button-functional" onClick={() => void openOrCreateJd()}>
              录入 JD
            </button>
            <button
              type="button"
            ...
            <button type="button" disabled={isGenerating} className="curator-button-functional" onClick={() => void generateJobDoc(jobFolderPath, "match")}>
              匹配分析
            </button>
            <button type="button" disabled={isGenerating} className="curator-button-functional" onClick={() => void generateJobDoc(jobFolderPath, "boss")}>
              BOSS 招呼语
            </button>
            <button type="button" disabled={isGenerating} className="curator-button-functional" onClick={() => void generateJobDoc(jobFolderPath, "email")}>
              求职邮件
            </button>
            <button type="button" disabled={isGenerating} className="curator-button-functional" onClick={() => void generateJobDoc(jobFolderPath, "custom-resume")}>
              定制简历
            </button>
            <button type="button" disabled={isGenerating} className="curator-button-functional" onClick={() => void generatePrepPack(jobFolderPath)}>
              面试准备包
            </button>
          </>
        ) : null}
        ...
        {interviewFolderPath ? (
          <button type="button" disabled={isGenerating} className="curator-button-functional" onClick={runReview}>
            生成复盘报告
          </button>
        ) : null}
      </div>
      ...
      <div className="mt-2 flex flex-wrap items-center gap-2 text-zinc-600">
        ...
        <button type="button" disabled={isGenerating} className="curator-button-danger px-2 py-1 text-xs" onClick={() => void prepareDeleteCurrentTarget()}>
          删除当前文件
        </button>
          ...
          <div className="flex min-w-0 flex-1 items-center justify-end gap-2">
            <div className="flex min-w-0 items-center gap-2 rounded-xl border border-white/75 bg-white/88 px-2 py-1 text-zinc-700 shadow-[0_10px_24px_rgba(148,163,184,0.1)]">
              ...
              {generationNotice.path ? (
                <button
                  type="button"
                  className="curator-button-secondary rounded-lg px-2 py-0.5 text-xs"
                  onClick={async () => {
              ...
              <button type="button" className="curator-button-secondary rounded-lg px-2 py-0.5 text-xs" onClick={() => clearGenerationNotice()}>
                关闭
              </button>
            </div>
          </div>
      ...
      <CuratorDialog
        ...
        footer={
          <>
            <button type="button" className="curator-button-secondary" onClick={() => setReviewDialogOpen(false)}>
              取消
            </button>
            <button
              type="button"
