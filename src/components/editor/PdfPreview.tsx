      ...
      <div className="border-b border-white/70 bg-white/72 px-4 py-3 backdrop-blur-xl">
        {isImportedResumePdf ? (
          <div className="flex flex-wrap items-center gap-2">
            <button type="button" className="curator-button-functional px-3 py-1 text-xs" disabled={Boolean(runningAction)} onClick={() => void handleGenerateMarkdown()}>
              {runningAction === "md" ? "生成中..." : "生成个人简历.md"}
            </button>
            <button
              type="button"
              className="curator-button-functional px-3 py-1 text-xs"
              disabled={Boolean(runningAction)}
              onClick={() => void handleGenerateMainResumeJson()}
            >
              {runningAction === "json" ? "生成中..." : "生成主简历 JSON"}
            </button>
            <a href={content} target="_blank" rel="noreferrer" className="curator-button-ghost px-3 py-1 text-xs">
              新窗口打开
            </a>
