...
import type { ResumeImportStage, ResumeJsonRunStatus } from "@/lib/resume-import";
...
import { getSparseResumeWarnings } from "@/lib/custom-resume-quality";
import { FeedbackSupportView } from "@/components/feedback/FeedbackSupportView";
...
const providerCatalog: Array<{
  ...
  },
  {
    ...
    models: ["qwen3.6-plus", "qwen3.5-plus", "qwen-plus", "qwen-turbo", "qwen-max"],
      ...
      {[1, 2, 3].map((step) => (
        <span
          ...
          className={`h-2.5 w-8 rounded-full transition-colors ${
            current > step ? "bg-sky-500" : current === step ? "animate-pulse bg-sky-400" : "bg-white/70"
          }`}
        />
      ))}
    </div>
...
function applyResumeRunStatusToState(
  status: ResumeJsonRunStatus,
  setProgress: (value: 1 | 2 | 3) => void,
...
const lightToggleButtonClassName =
  "inline-flex items-center rounded-full border border-white/75 bg-white/62 px-2.5 py-1 text-[11px] font-medium text-zinc-600 shadow-[inset_0_1px_0_rgba(255,255,255,0.78)] transition hover:border-sky-200/70 hover:bg-white/78 hover:text-sky-700";
...
function getLightToggleButtonClass(active: boolean) {
  return `${lightToggleButtonClassName} ${
    active
      ? "border-sky-200/90 bg-[linear-gradient(180deg,rgba(255,255,255,0.96),rgba(237,245,255,0.84))] text-sky-700 shadow-[0_10px_24px_rgba(59,130,246,0.08),inset_0_1px_0_rgba(255,255,255,0.8)]"
      : "text-zinc-600"
  }`;
}
...
function SaveBar({
  ...
  return (
    <div className="sticky top-0 z-10 flex items-center justify-between border-b border-line-hair bg-[rgba(255,255,255,0.72)] px-3 py-2 text-xs backdrop-blur-soft backdrop-saturate-150">
      <span className="text-text-muted">{title} · {text}</span>
      <button
        type="button"
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex flex-wrap gap-2">
        {items.map((item, idx) => (
          <span key={`${item}-${idx}`} className="inline-flex items-center gap-1 rounded-full border border-white/75 bg-white/82 px-2.5 py-1 text-xs text-zinc-600 shadow-[0_6px_14px_rgba(15,23,42,0.04)]">
            {item}
            <button
              type="button"
      ...
      <div className="flex gap-2">
        <input
          className="curator-input-surface flex-1 p-2 text-sm"
          value={input}
...
export function JsonFormView({ path }: { path: string }) {
  ...
  const [resumeImportStepDescription, setResumeImportStepDescription] = useState<string | null>(null);
  ...
  const [resumeStructureWarnings, setResumeStructureWarnings] = useState<string[]>([]);
  const [customResumeExporting, setCustomResumeExporting] = useState(false);
  const [customResumeNotice, setCustomResumeNotice] = useState<{
    type: "success" | "warning" | "error";
    text: string;
  } | null>(null);
  ...
  const isFeedbackSupportConfig = path === FEEDBACK_SUPPORT_PATH;
  const isCustomResumeJson = path.startsWith("/简历/定制简历/") && path.endsWith(".json");
  const customResumePreview = useMemo(() => {
    if (!isCustomResumeJson) return null;
    try {
      const parsed = JSON.parse(raw);
      const model = normalizeResume(parsed);
      return {
        model,
        parseError: "",
        sparseWarnings: getSparseResumeWarnings(model),
      };
    } catch {
      return {
        model: normalizeResume({}),
        parseError: "当前 JSON 格式有误，请先切换到原始模式修正后再导出。",
        sparseWarnings: [] as string[],
      };
    }
  }, [isCustomResumeJson, raw]);
  ...
  const resumeSourceStatus = useMemo<"md-ready" | "md-low-quality" | "md-blocked" | "md-draft" | "pdf-only" | "json-only" | "missing">(() => {
    const hasImportedPdf = Boolean(resumePdfFile?.content.trim());
    const resumeMdContent = resumeMarkdownFile?.content ?? "";
    const markdownState = getResumeMarkdownState(resumeMdContent, resumeMarkdownMeta);
  ...
  const resumeFlowStage = useMemo<ResumeFlowStage>(() => {
    ...
    if (resumeSourceStatus === "pdf-only" || resumeSourceStatus === "md-draft" || resumeSourceStatus === "md-blocked") return "pdf_saved";
    return "missing";
  ...
  useEffect(() => {
    ...
    setResumeImportStepDescription(null);
    setResumeImportProgress(1);
    setShowEmptyResumeForm(false);
    setRequireManualResumeSave(false);
    setResumeStructureWarnings([]);
    setCustomResumeNotice(null);
    setCustomResumeExporting(false);
  ...
  useEffect(() => {
    ...
    setResumeStructureWarnings(resumePrefillPayload.incompleteSections ?? []);
  ...
  async function verifyConnection() {
    const hasOwnConfig = Boolean(modelConfig.model.trim() && modelConfig.baseURL.trim() && modelConfig.apiKey.trim());
    if (!hasOwnConfig && !trialStatus?.trialEnabled) {
      setVerifyMessage({ type: "error", text: "当前既没有平台试用额度，也没有填写模型名、API Base URL 和 API Key。" });
      return;
    }
    if (hasOwnConfig && getInvalidUserApiConfigReason(modelConfig) === "api_key_non_latin1") {
      setVerifyMessage({
        type: "error",
        text: "连接失败：当前 API Key 中包含中文或全角字符，请检查是否误填了说明文字、示例文本或其它非密钥内容。",
      });
      return;
    }
    ...
    try {
      const result = await sendMessage({
        ...
        model: hasOwnConfig ? modelConfig.model.trim() : undefined,
        baseURL: hasOwnConfig ? modelConfig.baseURL.trim() : undefined,
        apiKey: hasOwnConfig ? modelConfig.apiKey.trim() : undefined,
  ...
  async function importResumeFromPdf(file: File) {
    ...
    setResumeImportStepDescription("先保存导入源文件，随后提取个人简历 Markdown。");
    try {
      ...
      setResumeImportStepDescription("正在把 PDF 转成可检查、可编辑的 Markdown 简历。");
      ...
      const persistedMarkdownMeta = parseResumeMarkdownMetadata(persistedMarkdownFile);
      const persistedMarkdownState = getResumeMarkdownState(persistedMarkdownContent, persistedMarkdownMeta);
        ...
        active_resume_source:
          persistedMarkdownState === "usable"
            ...
            : persistedMarkdownState === "low"
              ? "resume-markdown-low"
              : persistedMarkdownState === "blocked"
                ? "resume-markdown-blocked"
      ...
      if (hasConfiguredModel(llmConfig) && persistedMarkdownState === "usable") {
        applyResumeRunStatusToState(
          getResumeJsonRunStatus("structuring", "pdf"),
          setResumeImportProgress,
          setResumeImportStep,
          setResumeImportStepDescription,
        );
        ...
        const prefillResult = await generateMainResumeFromMarkdown({
          ...
          onStageChange: (status) =>
            applyResumeRunStatusToState(status, setResumeImportProgress, setResumeImportStep, setResumeImportStepDescription),
          runSource: "pdf",
  ...
  async function generateMainResumeFromMarkdownAction() {
    ...
    try {
      setResumeImporting(true);
      applyResumeRunStatusToState(
        getResumeJsonRunStatus("preparing", "markdown"),
        setResumeImportProgress,
        setResumeImportStep,
        setResumeImportStepDescription,
      );
      ...
      setShowEmptyResumeForm(true);
      setResumeStructureWarnings(result.incompleteSections ?? []);
      if (result.message) {
        setResumeImportMessage(result.message);
      }
    } catch (error) {
  ...
  const persistGenericJson = useCallback(async () => {
    if (isResume || isModelConfig || isJobCreateConfig || isFeedbackSupportConfig) return;
  ...
  }, [isFeedbackSupportConfig, isJobCreateConfig, isModelConfig, isResume, raw, saveRaw]);
  ...
  function getCustomResumePdfFileName() {
    const baseName = path.split("/").pop()?.replace(/\.json$/i, "").trim();
    const safeName = (baseName || "定制简历").replace(/[\\/:*?"<>|]/g, "-").trim();
    return `${safeName || "定制简历"}.pdf`;
  }

  async function exportCustomResumePdf() {
    if (!customResumePreview) return;
    if (customResumePreview.parseError) {
      setCustomResumeNotice({ type: "error", text: customResumePreview.parseError });
      return;
    }
    if (isResumeDataEmpty(customResumePreview.model)) {
      setCustomResumeNotice({
        type: "warning",
        text: "当前定制简历内容过于空缺，暂不建议导出 PDF。请先补充主简历或重新生成定制简历。",
      });
      return;
    }

    const fileName = getCustomResumePdfFileName();
    setCustomResumeExporting(true);
    setCustomResumeNotice(null);
    try {
      await exportResumePdf(customResumePreview.model, { fileName });
      setCustomResumeNotice({ type: "success", text: `已导出 PDF：${fileName}` });
    } catch (error) {
      setCustomResumeNotice({
        type: "error",
        text: `导出失败：${error instanceof Error ? error.message : "请重试"}`,
      });
    } finally {
      setCustomResumeExporting(false);
    }
      ...
      <div className="h-full overflow-auto">
        ...
        <div className="space-y-5 p-5">
          <div ref={tokenUsageSectionRef} className="glass-panel rounded-[24px] border border-white/80 bg-white/80 p-5">
            ...
            <div className="mt-3 rounded-[18px] border border-sky-100 bg-sky-50/80 px-4 py-3 text-xs text-sky-800">
              <p className="font-medium">平台试用状态</p>
              <p className="mt-1 leading-5">
                {trialStatus?.trialEnabled
                  ? `已启用平台试用：${trialStatus.provider || "平台模型"} ${trialStatus.model || ""}。剩余输入 ${formatTokenCount(trialStatus.remainingInputTokens)}，剩余输出 ${formatTokenCount(trialStatus.remainingOutputTokens)}。平台试用当前按 Token 与预算控制，不再按每日固定次数限制。`
                  : `当前未启用平台试用${trialStatus?.blockedReason ? `：${trialStatus.blockedReason}` : "，如需继续使用请填写你自己的 API。"}` 
                }
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="mt-1 flex gap-2">
              ...
              />
              <button type="button" className={getLightToggleButtonClass(showApiKey)} onClick={() => setShowApiKey((v) => !v)}>
                {showApiKey ? "隐藏" : "显示"}
              </button>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" className={compactFunctionalButtonClassName} onClick={() => void verifyConnection()} disabled={verifyLoading}>
              {verifyLoading ? "验证中..." : "验证连接"}
            </button>
            <button type="button" className={compactPrimaryButtonClassName} onClick={() => void saveModelConfig()} disabled={modelConfigStatus === "saving"}>
              保存配置
            </button>
          </div>
          ...
          <div className="glass-panel rounded-[24px] border border-white/80 bg-white/80 p-5">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => void loadLlmUsageRecords()}>
                刷新记录
              </button>
            </div>
          ...
          <div ref={systemPromptSectionRef} className="glass-soft p-3">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => void openSystemPrompt(SYSTEM_FILE_PATHS.global.prompt, "全局 Prompt")}>全局 Prompt</button>
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => void openSystemPrompt(SYSTEM_FILE_PATHS.global.agent, "全局 Agent")}>全局 Agent</button>
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => void openSystemPrompt(SYSTEM_FILE_PATHS.job.prompt, "岗位 Prompt")}>岗位 Prompt</button>
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => void openSystemPrompt(SYSTEM_FILE_PATHS.job.agent, "岗位 Agent")}>岗位 Agent</button>
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => void openSystemPrompt(SYSTEM_FILE_PATHS.resume.prompt, "简历 Prompt")}>简历 Prompt</button>
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => void openSystemPrompt(SYSTEM_FILE_PATHS.resume.agent, "简历 Agent")}>简历 Agent</button>
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => void openSystemPrompt(SYSTEM_FILE_PATHS.prep.prompt, "准备包 Prompt")}>准备包 Prompt</button>
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => void openSystemPrompt(SYSTEM_FILE_PATHS.prep.agent, "准备包 Agent")}>准备包 Agent</button>
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => void openSystemPrompt(SYSTEM_FILE_PATHS.review.prompt, "复盘 Prompt")}>复盘 Prompt</button>
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => void openSystemPrompt(SYSTEM_FILE_PATHS.review.agent, "复盘 Agent")}>复盘 Agent</button>
            </div>
            <div className="mt-3 border-t border-white/70 pt-3">
              <p className="text-sm font-medium">反馈与支持</p>
              <p className="mt-1 text-xs text-zinc-500">
                查看邮箱、群二维码，或直接留一句反馈。
              </p>
              <button
                type="button"
                className={`${compactFunctionalButtonClassName} mt-2`}
                onClick={() => void openFilePath(FEEDBACK_SUPPORT_PATH)}
              >
                反馈与支持
              </button>
            </div>
            <div className="mt-3 border-t border-white/70 pt-3">
              ...
              <p className="mt-1 text-xs text-zinc-500">可重新打开全屏向导，回看首次使用流程。</p>
              <button
                type="button"
                className={`${compactFunctionalButtonClassName} mt-2`}
  ...
  if (isFeedbackSupportConfig) {
    return <FeedbackSupportView path={path} raw={raw} />;
  }
        ...
        <div className="space-y-5 p-5">
          <div className="glass-panel rounded-[24px] border border-white/80 bg-white/80 p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => void openResumeImportForJobCreate()}>
                导入简历
              </button>
          ...
          <div className="flex flex-wrap gap-2">
            <button type="button" className={compactPrimaryButtonClassName} onClick={() => void submitJobCreate()} disabled={jobCreateStatus === "saving"}>
              保存并创建岗位
            </button>
            <button type="button" className={compactFunctionalButtonClassName} onClick={() => void saveJobCreateTemplate()} disabled={jobCreateStatus === "saving"}>
              仅保存表单
            </button>
          </div>
          {jobCreateMessage ? (
  ...
  if (isCustomResumeJson) {
    const parseError = customResumePreview?.parseError ?? "";
    const resumeModel = customResumePreview?.model ?? normalizeResume({});
    const sparseWarnings = customResumePreview?.sparseWarnings ?? [];

    return (
      <div className="h-full overflow-auto">
        <SaveBar title="定制简历 JSON" isDirty={jsonDirty} status={jsonSaveStatus} lastSavedAt={jsonLastSavedAt} onSave={persistGenericJson} />
        <div className="flex items-center justify-between border-b border-white/70 px-4 py-2 text-xs text-zinc-500">
          <span>默认可视化阅读，可切换原始 JSON 编辑</span>
          <div className="flex gap-1">
            <button
              type="button"
              className={compactPrimaryButtonClassName}
              onClick={() => void exportCustomResumePdf()}
              disabled={customResumeExporting}
            >
              {customResumeExporting ? "导出中..." : "导出 PDF"}
            </button>
            <button
              type="button"
              className={getLightToggleButtonClass(customResumeViewMode === "render")}
              onClick={() => {
            ...
            <button
              type="button"
              className={getLightToggleButtonClass(customResumeViewMode === "raw")}
              onClick={() => {
        ...
        {customResumeNotice ? (
          <div
            className={`mx-4 mt-3 rounded-[20px] border px-4 py-3 text-sm shadow-[0_14px_28px_rgba(15,23,42,0.06)] ${
              customResumeNotice.type === "success"
                ? "border-emerald-200 bg-emerald-50/90 text-emerald-700"
                : customResumeNotice.type === "warning"
                  ? "border-amber-200 bg-amber-50/90 text-amber-800"
                  : "border-red-200 bg-red-50/90 text-red-700"
            }`}
          >
            {customResumeNotice.text}
          </div>
        ) : null}
        {customResumeViewMode === "render" ? (
            ...
            {!parseError && sparseWarnings.length >= 4 ? (
              <div className="mb-4 rounded-[20px] border border-amber-200 bg-amber-50/85 px-4 py-3 text-sm text-amber-800 shadow-[0_14px_28px_rgba(251,191,36,0.08)]">
                ...
                <p className="mt-1 text-xs leading-5 text-amber-700">
                  目前缺少：{sparseWarnings.map((item) => `「${item}」`).join("、")}。建议先检查个人简历.md 或主简历内容，再重新生成定制简历。
                </p>
                <div className="mt-3">
                  <button
                    type="button"
                    className={compactFunctionalButtonClassName}
                    onClick={() => void openFilePath(resumeMarkdownFile?.content.trim() ? RESUME_MARKDOWN_PATH : RESUME_MAIN_JSON_PATH)}
                  >
                    {resumeMarkdownFile?.content.trim() ? "打开个人简历.md" : "打开主简历"}
                  </button>
                </div>
              </div>
            ) : null}
          ...
          <div className="p-4">
            <textarea
              className="curator-input-surface h-[calc(100vh-200px)] w-full p-3 font-mono text-xs"
              value={raw}
              onChange={(e) => {
  ...
  if (isResume) {
    ...
    const extractorLabel =
      resumeMarkdownMeta?.shouldBlockStructuring
        ? "提取结果不稳定"
        ...
        : resumeMarkdownMeta?.extractor === "pdfjs"
          ...
          : resumeMarkdownMeta?.extractor
            ? "文本提取"
            : null;
    const shouldBlockJsonGeneration = resumeSourceStatus === "md-blocked";
    const statusTone =
              ...
              <div className="mx-8 mb-4 rounded-[22px] border border-sky-100 bg-sky-50/80 px-5 py-4 text-left text-xs text-zinc-700">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="mt-1 text-zinc-500">{resumeImportStepDescription ?? "当前正在推进简历导入主链路。"}</p>
                  </div>
        ...
        <div className="space-y-5 p-5">
            ...
            <div className="mt-5 space-y-3">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  disabled={resumeImporting}
                  onClick={() =>
                    shouldBlockJsonGeneration
                      ? void openFilePath(RESUME_MARKDOWN_PATH)
                      : void generateMainResumeFromMarkdownAction()
                  }
                >
                  {shouldBlockJsonGeneration ? "先检查 Markdown" : "生成主简历 JSON"}
                </button>
              </div>
            ...
            {resumeImporting && resumeImportStep ? (
              <div className="mt-4 rounded-[22px] border border-sky-100 bg-sky-50/80 px-4 py-4 text-xs text-zinc-700">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    <p className="font-medium text-zinc-900">{resumeImportStep}</p>
                    <p className="mt-1 text-zinc-500">{resumeImportStepDescription ?? "正在推进主简历导入链路。"}</p>
                  </div>
                  <ResumeFlowDots current={resumeImportProgress} />
                </div>
              </div>
          ...
          {requireManualResumeSave ? (
            <div className="rounded-[24px] border border-amber-200 bg-amber-50/80 px-5 py-4 text-sm text-amber-800 shadow-[0_16px_32px_rgba(251,191,36,0.12)]">
              ...
              {resumeStructureWarnings.length > 0 ? (
                <p className="mt-2 text-xs leading-5 text-amber-700">
                  当前结构化仍不完整：{resumeStructureWarnings.map((item) => `「${item}」`).join("、")}。建议先检查
                  <button
                    type="button"
                    className="ml-1 inline-flex text-amber-800 underline underline-offset-2"
                    onClick={() => void openFilePath(RESUME_MARKDOWN_PATH)}
                  >
                    个人简历.md
                  </button>
          ...
          <div className="glass-panel grid grid-cols-2 gap-3 rounded-[24px] border border-white/80 bg-white/72 p-4 shadow-[0_18px_44px_rgba(15,23,42,0.06)] backdrop-blur-xl">
            <input className="curator-input-surface p-2 text-sm" placeholder="姓名" value={resume.profile.name} onChange={(e) => saveResumeToState({ ...resume, profile: { ...resume.profile, name: e.target.value } })} />
            <input className="curator-input-surface p-2 text-sm" placeholder="手机" value={resume.profile.phone} onChange={(e) => saveResumeToState({ ...resume, profile: { ...resume.profile, phone: e.target.value } })} />
            <input className="curator-input-surface p-2 text-sm" placeholder="邮箱" value={resume.profile.email} onChange={(e) => saveResumeToState({ ...resume, profile: { ...resume.profile, email: e.target.value } })} />
            <input className="curator-input-surface p-2 text-sm" placeholder="微信" value={resume.profile.wechat || ""} onChange={(e) => saveResumeToState({ ...resume, profile: { ...resume.profile, wechat: e.target.value } })} />
            <input className="curator-input-surface col-span-2 p-2 text-sm" placeholder="目标岗位（可选）" value={resume.profile.targetRole || ""} onChange={(e) => saveResumeToState({ ...resume, profile: { ...resume.profile, targetRole: e.target.value } })} />
          </div>

          <div className="glass-soft space-y-3 p-3">
            <div className="flex items-center justify-between">
              <p className="text-sm font-medium">教育经历</p>
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => saveResumeToState({ ...resume, education: [...resume.education, { school: "", degree: "", major: "", startDate: "", endDate: "", gpa: "" }] })}>
                添加教育经历
              </button>
            </div>
            {resume.education.length === 0 ? <p className="text-xs text-zinc-500">暂无教育经历</p> : null}
            {resume.education.map((item, idx) => (
              <div key={`edu-${idx}`} className="glass-soft space-y-2 p-3">
                <div className="grid grid-cols-2 gap-2">
                  <input className="curator-input-surface p-2 text-sm" placeholder="学校" value={item.school} onChange={(e) => updateEducation(idx, "school", e.target.value)} />
                  <input className="curator-input-surface p-2 text-sm" placeholder="学历" value={item.degree} onChange={(e) => updateEducation(idx, "degree", e.target.value)} />
                  <input className="curator-input-surface p-2 text-sm" placeholder="专业" value={item.major} onChange={(e) => updateEducation(idx, "major", e.target.value)} />
                  <input className="curator-input-surface p-2 text-sm" placeholder="GPA（可选）" value={item.gpa || ""} onChange={(e) => updateEducation(idx, "gpa", e.target.value)} />
                  <input className="curator-input-surface p-2 text-sm" placeholder="开始时间" value={item.startDate} onChange={(e) => updateEducation(idx, "startDate", e.target.value)} />
                  <input className="curator-input-surface p-2 text-sm" placeholder="结束时间" value={item.endDate} onChange={(e) => updateEducation(idx, "endDate", e.target.value)} />
                </div>
                <button type="button" className={compactDangerButtonClassName} onClick={() => saveResumeToState({ ...resume, education: resume.education.filter((_, i) => i !== idx) })}>
                  删除
                </button>
              </div>
            ))}
          ...
          <div className="glass-soft space-y-3 p-3">
            <div className="flex items-center justify-between">
              ...
              <button type="button" className={compactFunctionalButtonClassName} onClick={() => {
            ...
            {resume.internships.map((item, idx) => (
              <div key={`intern-${idx}`} className="glass-soft space-y-2 p-3">
                <div className="grid grid-cols-2 gap-2">
                  <input className="curator-input-surface p-2 text-sm" placeholder="公司" value={item.company} onChange={(e) => updateInternship(idx, "company", e.target.value)} />
                  <input className="curator-input-surface p-2 text-sm" placeholder="岗位" value={item.position} onChange={(e) => updateInternship(idx, "position", e.target.value)} />
                  <input className="curator-input-surface p-2 text-sm" placeholder="开始时间" value={item.startDate} onChange={(e) => updateInternship(idx, "startDate", e.target.value)} />
                  <input className="curator-input-surface p-2 text-sm" placeholder="结束时间" value={item.endDate} onChange={(e) => updateInternship(idx, "endDate", e.target.value)} />
                </div>
                <FieldLabel>工作描述（每行一条）</FieldLabel>
                <textarea className="curator-input-surface w-full p-2 text-sm" rows={4} value={getDraftText("internships", idx, item.descriptions)} onChange={(e) => updateInternship(idx, "descriptions", e.target.value)} />
                <button type="button" className={compactDangerButtonClassName} onClick={() => {
                  clearDraftTextsBySection("internships");
          ...
          <div className="glass-soft space-y-3 p-3">
            ...
            {resume.campusExperience.map((item, idx) => (
              <div key={`campus-${idx}`} className="glass-soft space-y-2 p-3">
                <div className="grid grid-cols-2 gap-2">
                  <input className="curator-input-surface p-2 text-sm" placeholder="组织/社团" value={item.organization} onChange={(e) => updateCampus(idx, "organization", e.target.value)} />
                  <input className="curator-input-surface p-2 text-sm" placeholder="角色" value={item.role} onChange={(e) => updateCampus(idx, "role", e.target.value)} />
                  <input className="curator-input-surface p-2 text-sm" placeholder="开始时间" value={item.startDate} onChange={(e) => updateCampus(idx, "startDate", e.target.value)} />
                  <input className="curator-input-surface p-2 text-sm" placeholder="结束时间" value={item.endDate} onChange={(e) => updateCampus(idx, "endDate", e.target.value)} />
                </div>
                <FieldLabel>描述（每行一条）</FieldLabel>
                <textarea className="curator-input-surface w-full p-2 text-sm" rows={4} value={getDraftText("campusExperience", idx, item.descriptions)} onChange={(e) => updateCampus(idx, "descriptions", e.target.value)} />
                <button type="button" className={compactDangerButtonClassName} onClick={() => {
                  clearDraftTextsBySection("campusExperience");
          ...
          <div className="glass-soft space-y-3 p-3">
            ...
            {(resume.projects ?? []).map((item, idx) => (
              <div key={`project-${idx}`} className="glass-soft space-y-2 p-3">
                <div className="grid grid-cols-2 gap-2">
                  <input className="curator-input-surface p-2 text-sm" placeholder="项目名" value={item.name} onChange={(e) => updateProject(idx, "name", e.target.value)} />
                  <input className="curator-input-surface p-2 text-sm" placeholder="角色" value={item.role} onChange={(e) => updateProject(idx, "role", e.target.value)} />
                </div>
                <FieldLabel>项目描述（每行一条）</FieldLabel>
                <textarea className="curator-input-surface w-full p-2 text-sm" rows={4} value={getDraftText("projects", idx, item.descriptions)} onChange={(e) => updateProject(idx, "descriptions", e.target.value)} />
                <FieldLabel>技术栈（逗号分隔）</FieldLabel>
                <input className="curator-input-surface w-full p-2 text-sm" value={(item.techStack ?? []).join(", ")} onChange={(e) => updateProject(idx, "techStack", e.target.value)} />
                <button type="button" className={compactDangerButtonClassName} onClick={() => {
                  clearDraftTextsBySection("projects");
          ...
          <div className="glass-soft space-y-4 p-3">
    ...
    <div className="h-full overflow-auto">
      ...
      <div className="flex items-center justify-between border-b border-white/70 px-4 py-2 text-xs text-zinc-500">
        ...
        <div className="flex gap-1">
          <button
            type="button"
            className={getLightToggleButtonClass(jsonViewMode === "render")}
            onClick={() => {
          ...
          <button
            type="button"
            className={getLightToggleButtonClass(jsonViewMode === "raw")}
            onClick={() => {
              setJsonViewMode("raw");
    ...
    <div className="space-y-4">
      ...
      {sections.map((section) =>
        section.items.length ? (
          <div key={section.title} className="glass-soft p-4">
