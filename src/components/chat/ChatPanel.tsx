    ...
    <div className="flex h-full flex-col">
      <div className="border-b p-2">
        <div className="mb-2 flex items-center gap-2">
          <button type="button" className="rounded-md border px-2 py-1 text-xs" onClick={() => void createThread()}>
            + 新对话
          </button>
          <select
        ...
        <div className="flex flex-wrap gap-1">
          ...
          {activeRefs.map((r) => (
            <ContextBadge key={r} label={`📄 ${r.split("/").pop()}`} removable onRemove={() => setActiveRefs((prev) => prev.filter((x) => x !== r))} />
          ))}
        </div>
      </div>
      ...
      <div ref={listRef} className="min-h-0 flex-1 space-y-2 overflow-auto p-2">
        ...
        {assistantStreaming ? <ChatMessage message={{ id: "__streaming__", threadId: currentThreadId ?? "", role: "assistant", content: assistantStreaming, timestamp: new Date().toISOString() }} /> : null}
      </div>
