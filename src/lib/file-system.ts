...
function isUniquePathConstraintError(error: unknown) {
  ...
  return (
    ...
    (message.includes("uniqueness") || message.includes("unique"))
  );
}
...
export async function createFile(
  ...
  try {
    ...
    if (!isUniquePathConstraintError(error)) {
      throw error;
    }

    const existing = await readFile(file.path);
    if (existing) {
      console.warn("[file-system] duplicate path create ignored", { path: file.path });
      return existing;
    }
