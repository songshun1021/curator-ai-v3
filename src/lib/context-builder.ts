...
import { buildAdaptiveInterviewStrategy, hasInterviewGrowthProfileContent, INTERVIEW_GROWTH_PROFILE_PATH } from "@/lib/interview-growth";
...
export async function getResumeSourceDiagnostics(jobFolderPath?: string): Promise<ResumeSourceDiagnostics> {
  ...
  const resumeMarkdownState = getResumeMarkdownState(resumeMdFile?.content, resumeMarkdownMetadata);
  ...
  return {
    ...
    hasAnyUsableSource,
    importedPdfDetectedButNotReadable:
      hasImportedPdf && (resumeMarkdownState === "missing" || resumeMarkdownState === "blocked") && !hasUsableStructuredResume,
    activeResumeSource,
  };
}
...
function pushResumeEvidence(
  ...
  args: {
    ...
    resumeMdFile?: { path: string; content: string; metadata?: string } | null;
  },
  options?: {
    mode?: "default" | "custom-resume";
  },
) {
  const evidenceMode = options?.mode ?? "default";
  const markdownState = getResumeMarkdownState(args.resumeMdFile?.content, parseResumeMarkdownMetadata(args.resumeMdFile));
  ...
  const hasUsableMarkdown = markdownState === "usable" || markdownState === "low";

  if (evidenceMode === "custom-resume") {
    if (hasUsableMarkdown && hasUsableStructured) {
      messages.push({
        role: "user",
        content: makeUserBlock(
          ...
          [
            ...
            "1. 岗位 JD",
            "2. 个人简历.md 中的原始文本事实",
            "3. 主简历/绑定简历 JSON 中的结构化基线",
            "若 Markdown 与结构化 JSON 都存在，必须同时参考两者；优先保留两者共同确认的事实，不要凭空改写日期、学历、岗位、项目或技能。",
          ].join("\n"),
        ),
      });
    }
    ...
    if (hasUsableMarkdown && !hasUsableStructured) {
      messages.push({
        ...
        content: makeUserBlock(
          "定制简历来源说明",
          "当前没有可用的结构化主简历 JSON，本次定制将主要基于 个人简历.md 的原始事实和岗位 JD 生成。",
        ),
      });
    }
    ...
    messages.push({
      ...
      content: makeUserBlock(
        "定制简历贴岗改写步骤",
        [
          "1. 先从 JD 提炼 3-5 个核心能力或职责信号。",
          "2. 再从已有简历事实里逐条找对应证据。",
          "3. 再按相关性重排经历：高相关前置，低相关降权。",
          "4. 让 HR 在 10-15 秒扫读首屏时，就能看出你为什么值得给面试。",
          "5. bullet 优先写清动作、结果、量化数据和业务场景，少写空泛职责。",
          "6. 最后重写描述，让表述更贴岗位语言，但不得新增原简历中不存在的事实。",
        ].join("\n"),
      ),
    });
    ...
    if (hasUsableMarkdown) {
      ...
      if (markdownState === "low") {
        messages.push({
          ...
          content: makeUserBlock(
            "简历证据说明",
            "当前个人简历文本较短，请结合岗位 JD 与结构化简历 JSON 一起交叉校验，只基于已有事实裁剪，不要编造经历或结果。",
          ),
        });
      }
    }
    ...
    if (hasUsableStructured) {
      const structuredLabel =
        hasBoundStructured && args.requestedResumePath !== RESUME_MAIN_JSON_PATH
          ? `绑定简历JSON（${args.resumeFile?.path || args.resumePath}）`
          : `主简历JSON（${args.resumeFile?.path || args.resumePath}）`;
      messages.push({
        role: "user",
        content: makeUserBlock(structuredLabel, args.resumeFile?.content ?? ""),
      });
    }
    return;
  }
...
export async function buildContext(args: {
  ...
  userPrompt?: string;
  resumeEvidenceMode?: "default" | "custom-resume";
  ...
  const growthProfile = (await readFile(INTERVIEW_GROWTH_PROFILE_PATH))?.content ?? "";
  const adaptiveStrategy = buildAdaptiveInterviewStrategy(growthProfile);
  ...
  if (args.jobFolderPath) {
    ...
    pushResumeEvidence(messages, resumeBlocks, { mode: args.resumeEvidenceMode ?? "default" });
  }
  ...
  if (args.mode === "prep-pack") {
    ...
    if (hasInterviewGrowthProfileContent(growthProfile)) {
      messages.push({ role: "user", content: makeUserBlock("面试成长画像", growthProfile) });
    }
    if (adaptiveStrategy) {
      messages.push({ role: "user", content: makeUserBlock("动态成长策略块", adaptiveStrategy) });
    }
  }
