...
import { getInterviewGrowthProfileTemplate, INTERVIEW_GROWTH_PROFILE_PATH } from "@/lib/interview-growth";
import { FEEDBACK_SUPPORT_PATH, getDefaultFeedbackSupportConfig, serializeFeedbackSupportConfig } from "@/lib/feedback";
...
async function ensureCoreWorkspaceFiles() {
  ...
  await createJson(
    FEEDBACK_SUPPORT_PATH,
    "/AI配置",
    serializeFeedbackSupportConfig(getDefaultFeedbackSupportConfig()),
    false,
  );
  await createMd(INTERVIEW_GROWTH_PROFILE_PATH, "/AI配置", getInterviewGrowthProfileTemplate(), false);
}

async function runWorkspaceInitialization(): Promise<void> {
  console.info("[init-workspace] start");
  const count = await db.files.count();

  if (count === 0) {
    console.info("[init-workspace] bootstrapping empty workspace");
    await createFolder("/简历", "/");
    await createFolder("/岗位", "/");
    await createFolder("/面试准备包", "/");
    await createFolder("/面试复盘", "/");
  ...
  console.info("[init-workspace] complete");
}

export async function initWorkspace(): Promise<void> {
  if (workspaceInitPromise) {
    console.info("[init-workspace] reuse in-flight initialization");
    return workspaceInitPromise;
  }

  workspaceInitPromise = runWorkspaceInitialization().finally(() => {
