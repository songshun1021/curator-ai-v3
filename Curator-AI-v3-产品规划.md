...
为避免“按错文档开发”，以下条款自本日期起优先级高于本文其他冲突描述：

1. 系统文件路径统一为 `/.system/*`：`prompt.md`、`agent.md`；旧 `_*.prompt.md/_*.skill.md` 仅兼容读取，不再作为主路径。
2. 文件树默认隐藏系统文件（`isSystem=true` 或 `/.system/`）；通过 AI 配置页“高级入口”进行编辑。
3. 渲染策略统一为“阅读优先”：`.md` 与 `.json` 默认渲染视图，保留一键切换原始编辑。
4. 产物格式规则统一：
   - 文书/报告类：Markdown（`.md`）
   - 结构化资产：JSON（例如定制简历 `.json`），并要求严格合法 JSON
   - 聊天气泡：纯文本渲染
5. 技术栈以仓库现状为准：Next.js 14 + React 18 + Tailwind CSS v3 + pnpm；React 19/Tailwind v4 仅列为后续里程碑。
6. 本轮基线治理不改 `/api/chat` 协议、不改 Dexie schema、不引入后端新依赖。
...
## 2026-04-10 补充：面试复盘多轮次目录（基线修订）

- 复盘目录从单层升级为：`/面试复盘/{公司-岗位}/{轮次}/`。
- 每个轮次固定文件：`面试原文.md`、`复盘报告.md`、`meta.json`。
- 支持同岗位多轮次独立沉淀（如一面、二面、HR面），互不覆盖。
- 历史旧目录 ` /面试复盘/{公司-岗位-轮次} ` 启动时自动迁移到新结构。
- 迁移策略：新路径优先、旧路径兜底，迁移异常不阻断主流程。
